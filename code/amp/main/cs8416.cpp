...
CS8416::CS8416( uint8_t address, I2CBUSPtr bus ) : mAddress( address ), mRunning( false ), mInput( 0 ), mCurrentFormat( 0 ), mChipVersion( 0 ) {
    mBus = bus->bindAddress( address );
}
...
CS8416::init() {
    ...
    mBus->writeRegisterByte( CS8416::ADDR_CONTROL_0, SPDIF_PDUR );

    // no auto clock switching, no muted zeros, active high interrupts, error switch to zeros,
    // master clock is 256*fs
    mBus->writeRegisterByte( CS8416::ADDR_CONTROL_1, SPDIF_ERROR_ZERO );

    // DETCI?, de-emphasis off (turn it on after?), GPIO0 is sample rate
    mBus->writeRegisterByte( CS8416::ADDR_CONTROL_2, 8 );

    // GPIO1 is non-audio detection, GPIO2 is GND
    mBus->writeRegisterByte( CS8416::ADDR_CONTROL_3, ( 0b1001 << 4 ) | 0b0000 );

    // output as a I2S master, 24 bit audio, left justified data, 128*fs was 0b11000000 

    // Left justified
    //mBus->writeRegisterByte( CS8416::ADDR_SERIAL_DATA_FORMAT, SPDIF_MASTER | SPDIF_FORMAT_LJ );

    // I2S 
    mBus->writeRegisterByte( CS8416::ADDR_SERIAL_DATA_FORMAT, SPDIF_MASTER | SPDIF_FORMAT_I2S );

    // Error mask - give us information about PLL locking, PCM validity, and confidence
    mBus->writeRegisterByte( CS8416::ADDR_RECEIVER_ERROR_MASK, 0b00011100 );
}
...
CS8416::updateRunCommand() {
    if ( mRunning ) {
        ...
        uint8_t control = SPDIF_RUN | ( mInput << 3 );
        mBus->writeRegisterByte( CS8416::ADDR_CONTROL_4, control );
    } else {
...
CS8416::getBitDepth() {
    uint8_t value = 0;
    mBus->readRegisterByte( CS8416::ADDR_CHANNEL_A_BYTE_4, value );
    AMP_DEBUG_I( "SPDIF raw channel 4 data is %#02x", value );
...
CS8416::getSamplingRate() {
    uint8_t value = 0;
    mBus->readRegisterByte( CS8416::ADDR_CHANNEL_A_BYTE_3, value );
    switch( value & 0x0f ) {
        case 0:
            return 44100;
        case 2:
...
CS8416::getVersion() {
    uint8_t version = 0;
    mBus->readRegisterByte( CS8416::ADDR_ID_VERSION, version );
...
CS8416::getFormat() {
    uint8_t format = 0;
    mBus->readRegisterByte( CS8416::ADDR_CONTROL_4, format );
...
CS8416::getErrorInfo() {
    uint8_t status = 0;
    mBus->readRegisterByte( CS8416::ADDR_RECEIVER_ERROR, status );
    return status; 
}
